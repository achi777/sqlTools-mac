...
- **AND-combined** across columns (Navicat default). Filtered columns are
  highlighted (⚑) and a **Clear filters (N)** button appears.
...
### Visual filter builder (nested AND/OR)

For anything beyond a flat AND of quick filters, the grid toolbar's **Filter
builder** opens a visual editor for arbitrarily **nested groups**:
...
- Each **group** has an AND/OR toggle and an optional **NOT**; add **conditions**
  ...
  OR NOT (vip = true)`.
- Conditions are **type-aware** (same operator sets as the quick filter, plus
...
- **Compiler:** a single pure `compileFilter(engine, quickFilters, tree, …)`
  ...
  and — like the quick filter — binds every value and escapes LIKE wildcards.
...
**Coexistence rule (documented):** the **effective filter = (quick filters
AND-combined) AND (builder tree)** — they **intersect**. Both show indicators
(⚑ columns for quick, a • on the Filter builder button when active), and a
single **Clear all filters** removes both. The combined filter feeds the same
paginated query + COUNT, so "Rows X–Y of N" and deterministic pages stay correct.
The builder filter is per-tab and not persisted across restart (noted as
future work).
