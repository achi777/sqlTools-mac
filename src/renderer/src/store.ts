...
import { PAGE_SIZE_DEFAULT } from '@shared/types'
...
export type TreeTarget =
  ...
  | { kind: 'trigger'; connectionId: string; schema: string; table: string; name: string }
...
export interface ObjectEditorState {
  objKind: 'view' | RoutineKind
  mode: 'new' | 'edit'
  schema: string
  /** View: editable target name. Routine: original name for display/drop. */
  name: string
  signature?: string | null
  /** View: the SELECT body. Routine: the full CREATE statement. */
  body: string
...
interface TreeState {
  ...
  /** Tables whose child nodes (Triggers/Indexes) are expanded — keys `${schema}::${table}`. */
...
export type ObjCategory = 'views' | 'functions' | 'procedures' | 'sequences'
...
function defaultTriggerSpec(engine: Engine, schema: string, table: string): TriggerSpec {
  ...
  return {
    ...
    name: `_trg_${table}`,
    originalName: null,
    timing: engine === 'postgres' ? 'BEFORE' : 'AFTER',
    event: 'INSERT',
    level: 'ROW',
    body: engine === 'mysql' ? myBody : engine === 'sqlite' ? liteBody : '',
...
function effectiveFilter(t: Tab): { filters: ColumnFilter[]; tree: FilterGroup | null; customWhere: string | null } {
  return {
    filters: t.filterMode === 'quick' ? t.filters : [],
    tree: t.filterMode === 'builder' ? t.builderTree : null,
    customWhere: t.filterMode === 'custom' ? (t.customWhere.trim() || null) : null
  }
}
...
function emptyTableSpec(schema: string): TableSpec {
  return {
    schema,
    name: 'new_table',
    columns: [
      { name: 'id', type: 'integer', nullable: false, autoIncrement: true, originalName: null },
      { name: 'name', type: 'varchar', length: 255, nullable: false, originalName: null }
    ],
    primaryKey: ['id'],
    foreignKeys: [],
    indexes: []
...
export const useStore = create<AppState>((set, get) => {
  ...
  return {
    ...
    toggleSchema: async (connId, schema) => {
      ...
      let expanded: string[]
      let tablesBySchema = tree.tablesBySchema
      if (isOpen) {
        expanded = tree.expanded.filter((s) => s !== schema)
      } else {
        expanded = [...tree.expanded, schema]
        if (!tablesBySchema[schema]) {
          const res = await window.dbApi.listTables(connId, schema)
          if (res.ok) tablesBySchema = { ...tablesBySchema, [schema]: res.data }
        }
      }
      set({
        treeByConn: {
          ...get().treeByConn,
          [connId]: { ...tree, expanded, tablesBySchema }
        }
      })
    },
    ...
    setColumnFilter: async (column, filter) => {
      ...
      updateTab(at.id, { filters, filterMode: 'quick' })
    ...
    setBuilderTree: async (tree) => {
      const at = get().getActiveTab()
      if (!at?.gridTable) return
      updateTab(at.id, { builderTree: tree, filterMode: 'builder' })
      await get().reloadFiltered()
    },

    // "Clear" clears ONLY the active mode's filter (others are retained).
    clearAllFilters: async () => {
      const at = get().getActiveTab()
      if (!at?.gridTable) return
      if (at.filterMode === 'quick') updateTab(at.id, { filters: [] })
      else if (at.filterMode === 'builder') updateTab(at.id, { builderTree: null })
      else updateTab(at.id, { customWhere: '', customWhereError: null })
      await get().reloadFiltered()
    },
    ...
    toggleCategory: async (connId, schema, cat) => {
      ...
      if (!isOpen) {
        if (cat === 'views' && !next.viewsBySchema[schema]) {
    ...
    applyObjectEditor: async (tabId) => {
      ...
      const cat: ObjCategory = oe.objKind === 'view' ? 'views' : oe.objKind === 'procedure' ? 'procedures' : 'functions'
    ...
    applySequenceEditor: async (tabId) => {
      ...
      try {
        const preview =
          se.mode === 'new' ? buildCreateSequence(se.spec) : buildAlterSequence(se.spec, se.original as SequenceSpec)
